...
### 4. Зависимость краша.
У нас есть кластер из нескольких сервисов. Все они обращаются к одной и той же БД, но у каждого сервиса своя схема. Каждый сервис получает и записывает данные только в свою схему. В какой-то момент было принято решение добавить новый микросервис. Для него тоже была создана схема в базе данных. И вот после запуска нового сервиса, один из старых перестал запускаться. Оказалось, что вместе в новым сервисом суммарное количество подключений к БД превысило максимальное. 
Проблему решили увеличением максимума подключений к БД.
