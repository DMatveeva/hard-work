...
1. У нас есть сущность Hibernate Task с полем user, которое загружается лениво. Методы toString, equals, hashCode реализованы через аннотации Lombok. 
```java
import lombok.*;
...
Первая - в методе getTask(), где находим в БД Task и хотим залогировать.
...
2. В проекте используется библиотека quickfix, для интеграции по протоколу FIX с другим сервисом. Во время работы сервисы устанавливают сессию, и FIX-сервер присылает котировки нашему FIX-клиенту. 
И вдруг спустя несколько месяцев нормальной работы вдруг сессия стала разрываться. Восстановление сессии происходит автоматически, но занимает до пары минут, что критично для системы, т.к. клиент пересылает котировки в третью систему, где есть требования к частоте получения котировок. 
...
Оказалось, дело в том, что команда devOps сделала новую балансировку кластера, где развернут сервис B. Работоспособность всего кластера стала определяться по другому сервису С, который в тот момент был недоступен. 
Т.к. другая команда настроила эту балансировку, не уведомив нас, ответственность за ошибку была на них. 

4. Найдите и подправьте три места в ваших проектах, где например с помощью продуманного типа результата функции/метода можно явно исключить нежелательные формы обработки этого результата. Часто возможно и желательно использовать пользовательский тип результата.


Три места, где с помощью продуманного типа результата метода можно явно исключить нежелательные формы обработки этого результата (желательно и возможно использовать пользовательский тип результата)

1. 
2. Метод репозитория возвращал List<>

1. Найти пример, когда я полагалась на то что возвращаемые значения упорядочены по времени, а они не были
2. Когда может вернуть нал?
